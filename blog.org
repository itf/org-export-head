...
(defun get-hash-value-content()
  ...
  (hash-function (org-export-head--get-content-subtree-at-point)))

(defun hash-function (text)
  (md5 text))
...
(defun org-export-head--update-hashes()
  ...
  (org-export-head--run-on-each-heading 
   #'(lambda()
       (let ((new-hash  (format "%s" (get-hash-value-content)))
...
(defun org-export-head--get-headlines-ast (ast)
  (flet ((make-hash ()
    ...
    (let ((set (make-hash)))
      (org-export-head--run-on-not-match-headline-ast
       #'(lambda(headline) (add-to-hash headline set))
       '("noexport") ast)
      set)))
